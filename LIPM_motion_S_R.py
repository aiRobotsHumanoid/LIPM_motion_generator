...
write_data = True
motion_file = "./source/motordata/Shift_R_data.csv"    # LIPM給模擬環境的動作檔 
w_file = "./Motion_cmd/Motion_Shift_R.csv" # motor的刻度檔案
...
class LIPM_motion:
    def __init__(self):
        ...
        self.B1 = -44.5 * 0.001 # (+)往內畫 (-)往外畫 -0.04 
        self.B2 = 40 * 0.001  # (+)往外畫 (-)往內畫

        # Hip側開
        self.h1 = 0.8    # 右腳hip (+)抬左腳更低, 重心偏右 (-)抬左腳更高  -0.8 
        self.h2 = -0.95      # 左腳hip (+)抬右腳更低, 重心偏左 (-)抬右腳更高   0.9
        ...
        # 腳底板傾斜角度, (+)腳尖往上 (-)腳尖往下
        self.CR1 = 1 # (-)腳尖往下 (+)腳尖往上
        self.CR2 = 0 
        self.CL1 = -5  # (+)腳尖往下 (-)腳尖往上
        self.CL2 = 0

        # Lean angle sequence [[step1], [step2]]
        # self.A_Lean_angle_R = [[0.8*self.CR1, self.CR1, self.CR1, self.CR1, 0.6*self.CR1+self.CR2*0.4], \
        #                         [self.CR1*0.3+self.CR2*0.7, self.CR2, self.CR2*0.5, self.CR2*0.25, 0]] # 夾角變大腳尖落地
        self.A_Lean_angle_R = [[0.5,  2,     7.7,      5,      2],\
                               [-2,       -3,       -1,      -1,     0]]  # ???[1,  10,     12,      9,      3] [0,       0,       -2,      -1,     0]
        self.A_Lean_angle_L = [[0, 1.9, 1.5,  1, 0], \
                               [0.5,  0.5,  0,    0, 0 ]]
        ...
        self.foot_height = 0.51
        self.zc = 0.45 #0.45           # zc: 質心高度，影響LIPM產生的質心軌跡 
        self.CoMx_bias = 0.            # CoMx_bias: 質心前後偏移量

        """ 動作時間設定 """
        self.footstep = 2
        self.k_DSP = 0.44               # 值越小，抬腳的時間越長
