...
class LIPM_motion:
    def __init__(self):
        """ LIPM參數設定, 1右 2左 """
        # 大腿擺幅, |0.05| < B < |0.1|
        self.B1 = -111* 0.001  # (+)往內畫 (-)往外畫 -0.04 
        self.B2 = 105 * 0.001  # (+)往外畫 (-)往內畫

        # Hip側開
        self.h1 = 0.30    # 右腳hip (+)抬左腳更低, 重心偏右 (-)抬左腳更高  -0.8
        self.h2 = 0.31   # 左腳hip (+)抬右腳更低, 重心偏左 (-)抬右腳更高   0.9
        ...
        # Lean angle sequence [[step1], [step2]]
                            #    1      2       3       4       5                   
        self.A_Lean_angle_R = [[ -5,   -8,    -9,    -10,     -10], \
                               [ -7,      -4,    -3,   -3,     -5 ] ]
        self.A_Lean_angle_L = [[ -1.5,     -1.5,   -1.5,   -1.4,     -0.8], \
                               [-0.3,    -1,    -1,    -1.0,      -1.3]]
