...
write_data = True
motion_file = "./source/motordata/cforward_data2.csv"    # LIPM給模擬環境的動作檔 
w_file = "./Motion_cmd/Motion_CForward2.csv" # motor的刻度檔案
...
class LIPM_motion:
    def __init__(self):
        """ LIPM參數設定, 1右 2左 """
        # 大腿擺幅, |0.05| < B < |0.1|
        self.B1 = -110* 0.001  # (+)往內畫 (-)往外畫 -0.04 
        self.B2 = 92 * 0.001  # (+)往外畫 (-)往內畫

        # Hip側開
        self.h1 = 0.15    # 右腳hip (+)抬左腳更低, 重心偏右 (-)抬左腳更高  -0.8
        self.h2 = 0.54   # 左腳hip (+)抬右腳更低, 重心偏左 (-)抬右腳更高   0.9
        ...
        # Lean angle sequence [[step1], [step2]]
                            #    1      2       3       4       5                   
        # self.A_Lean_angle_R = [[ -2,   -6,    -12,    -15,     -15], \
        #                        [ -10,      -6,    -3,   -3,     -2 ] ]           
        self.A_Lean_angle_R = [[ 1,   0,    -8,    -10,     -8], \
                                [ -10,      -3.,   0.,   1,     1]]  # [ -1,      -0,   0.5,   1,     1.2][ -5,      -4,   -1.5,   0,     1] [ -4,      -3,   -2,   0,     1]
        self.A_Lean_angle_L = [[ -1,     -3.5,   -2.5,   0.0,     0.5], \
                               [-1,    -1,    -1,    -1.2,      -1]]
        ...
        """ 機器人基礎姿態設定 """
        # Set Initial Pose
        self.initR = [0,  -0.00,   -0.37, -0.77,  0.41,  0] 
        self.initL = [0,  -0.00,    0.48,  0.73,  0.30,  0]
