...
write_data = True
motion_file = "./source/motordata/Turn_R_data.csv"    # LIPM給模擬環境的動作檔 
w_file = "./Motion_cmd/Motion_Turn_R.csv" # motor的刻度檔案
...
class LIPM_motion:
    def __init__(self):
        """ LIPM參數設定, 1右 2左 """
        # 大腿擺幅, |0.05| < B < |0.1|
        self.B1 = -0.070  # (+)往內畫 (-)往外畫 -0.04 
        self.B2 = 0.09  # (+)往外畫 (-)往內畫
        ...
        # Lean angle sequence [[step1], [step2]]
                            #    1      2       3       4       5                   
        self.A_Lean_angle_R = [[ 2.0,   -1,    0,    -3,     -1], \
                               [ 0,      3.00,    3.0,   5,     4.5 ] ]
        self.A_Lean_angle_L = [[ -0.8,     -0.6,   -0.4,   -0.4,     0], \
                               [-0.3,    -2,    -2,    -1.5,      -1.2]]
