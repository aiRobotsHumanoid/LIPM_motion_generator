...
class LIPM_motion:
    def __init__(self):
        """ LIPM參數設定, 1右 2左 """
        # 大腿擺幅, |0.05| < B < |0.1|
        self.B1 = -0.08 # (+)往內畫 (-)往外畫 -0.04 
        self.B2 = 0.085  # (+)往外畫 (-)往內畫

        # Hip側開
        self.h1 = 0.58    # 右腳hip (+)抬左腳更低, 重心偏右 (-)抬左腳更高  -0.8
        self.h2 = 0.75    # 左腳hip (+)抬右腳更低, 重心偏左 (-)抬右腳更高   0.9
        ...
        # Lean angle sequence [[step1], [step2]]
                            #    1      2       3       4       5                   
        self.A_Lean_angle_R = [[ 3.00,   3.00,    0.40,    1.00,     0.2], \
                               [ 0.5,      2.00,    4.0,   3.0,      2 ] ]
        self.A_Lean_angle_L = [[ 0,     -0.75,   -1.20,   -1.45,     -1.9], \
                               [-1,    -1.5,    -1.0,    -0.5,      0.0]]
